...
def parseFile(filename):
    ...
    for line in file:
      ...
      similarities[(word1, word2)] = human_score
    ########################
    return similarities
...
def linSimilarities(word_pairs, ic):
    ...
    for word_pair in word_pairs:
      word1 = word_pair[0]
      word2 = word_pair[1]
      w1_noun, w1_verb = False, False
      w2_noun, w2_verb = False, False
      if(len(wn.synsets(word1,'n')) > 0):
	w1 = wn.synsets(word1, 'n')[0]
	w1_noun = True
      else:
	w1 = wn.synsets(word1, 'v')[0]
	w1_verb = True
      if(len(wn.synsets(word2,'n')) > 0):
	w2 = wn.synsets(word2, 'n')[0]
	w2_noun = True
      else:
	w2 = wn.synsets(word2, 'v')[0]
	w2_verb = True
      if(w1_noun == True and w2_noun == False):
	w1 = wn.synsets(word1, 'v')[0]
      if(w1_noun == False and w2_noun == True):
	w2 = wn.synsets(word2, 'v')[0]
      similarities[word_pair] = float(w1.lin_similarity(w2, ic) * 10)
    ########################
    return similarities
...
def resSimilarities(word_pairs, ic):
	...
	w1 = wn.synsets(word1, 'v')[0]
      if(w1_noun == False and w2_noun == True):
	w2 = wn.synsets(word2, 'v')[0]
      similarities[word_pair] = w1.res_similarity(w2, ic)
    ########################
    return similarities
...
def vecSimilarities(word_pairs, model):
	...
	try:
	  similarities[word_pair] = float(model.similarity(word1, word2)*10)
	except KeyError:
	  similarities[word_pair] = 0
    ########################
    return similarities
...
def main():
    ...
    #for x in human_sims.keys():
    #  print x, ' : ', human_sims[x]
