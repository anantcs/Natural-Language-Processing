...
class nmt_dynet_attention:
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, src_word2idx, src_idx2word, tgt_word2idx, tgt_idx2word, word_d, gru_d, gru_layers):
        ...
        # attention weights
        self.attention_w1 = self.model.add_parameters((2*self.gru_d+self.word_d, 2*self.gru_d))
        self.attention_w2 = self.model.add_parameters((2*self.gru_d+self.word_d, self.gru_d))
        self.attention_v = self.model.add_parameters((1, 2*self.gru_d+self.word_d))
        ...
        # decoder network
        self.dec_RNN = RNN_BUILDER(self.gru_layers, self.gru_d*2 + self.word_d, self.gru_d, self.model)
    ...
    def decode(self, encoded_sentence, tgt_sentence):
        ...
        s = s.add_input(vecInput(2*self.gru_d+self.word_d))
        loss = []

        for word in tgt_sentence:
            attention  = self.attend(encoded_sentence, s)
            w = parameter(self.output_w)
            b = parameter(self.output_b)
            vector = concatenate([attention, last_tgt_embeddings])
    ...
    def attend(self, input_vectors, state):
        ...
        for vector in input_vectors:
            curr_state = parameter(self.attention_w1)*vector
            prev_state = parameter(self.attention_w2)*state.s()[-1]
            temp.append(parameter(self.attention_v)*tanh(curr_state + prev_state))

        attention_weights = concatenate(temp)
        attention_weights = softmax(attention_weights)
        ct = [v1 * v2 for v1, v2 in zip(input_vectors, attention_weights)]
        return esum(ct)
...
def main(train_src_file, train_tgt_file, dev_src_file, dev_tgt_file, model_file, num_epochs, embeddings_init = None, seed = 0):
    ...
    print 'Initializing simple neural machine translator with attention:'
