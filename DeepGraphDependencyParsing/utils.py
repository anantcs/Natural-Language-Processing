...
class Vocabulary:
    ...
    def process(self, data, deterministic=False):
        ...
        for i in range(len(data)):
            ...
            for j in range(len(data[i])):
                ...
                if deterministic == False:
                    prob_x = random.uniform(0, 1)
                    prob_y = 0.25/(0.25+self.words[word])
                    if prob_x <= prob_y:
                        word = UNKNOWN
                index.append(self.word2idx[word])
                pos_index.append(self.pos2idx[data[i][j].pos])
                g_arc.append(data[i][j].parent_id)
                if data[i][j].relation in self.rel2idx:
                    dep_rel.append(self.rel2idx[data[i][j].relation])
                else:
                    dep_rel.append(0)
        ...
        return indices, pos_indices, g_arcs, dep_rels 
